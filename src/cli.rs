...
- For hash/crypto findings, do not mark vulnerable only because `MessageDigest.getInstance`, `Cipher.getInstance`, or a generic benchmark execution banner is reachable. Use java_crypto_semantic_scan when a Java source file or benchmark case ID is available, and require weak algorithm/mode evidence such as MD5, SHA-1, DES, RC4, ECB, or provider-default AES before returning vulnerable; if only the generic runtime banner is available, return inconclusive.
- For weak randomness benchmark cases, do not mark vulnerable only because a generic Weak Randomness runtime banner is reachable. Use java_randomness_semantic_scan when a Java source file or benchmark case ID is available; java.util.Random, Math.random, or ThreadLocalRandom support vulnerable, while SecureRandom supports not_vulnerable unless predictable seeding is shown.
- For SQL injection, LDAP injection, XPath injection, and trust-boundary benchmark cases with collapsed fixed error pages or no runtime input delta, use java_injection_semantic_scan when a Java source file or benchmark case ID is available. Tainted request data flowing into SQL/LDAP/XPath/session sinks is vulnerable evidence; constant/safe-helper values reaching the sink are not_vulnerable evidence. For trust-boundary cases, a reachable session write alone is not enough: require user-controlled session key/value influence or source-level taint into setAttribute/putValue.
- For xss_risk_scan, do not call it with only a bare URL. It needs at least one testable input point: query params in the URL, query_params, object body fields, injectable_fields, or injectable_headers. For bare authorized benchmark/lab XSS cases with no visible form, include bounded header probes such as `referer`, `user-agent`, and `x-forwarded-for`.
